...
public class MainMenu : MonoBehaviour
{
	...
	void OnGUI ()
	{
			...
			if (!showYesNoPrompt) {
					if (GUI.Button (new Rect (Screen.width * .575f, Screen.height * .35f, Screen.width * .2f, Screen.height * .1f), "New Game", oppaFontStyle)) {
							//print ("Clicked Play Game");
							Application.LoadLevel (Application.loadedLevel + 1); //theoretically this should be the first level
					}
					if (GUI.Button (new Rect (Screen.width * .575f, Screen.height * .5f, Screen.width * .2f, Screen.height * .1f), "Credits", oppaFontStyle)) {
				//print ("Clicked Play Game");
							Application.LoadLevel ("Credits"); //go to credits
				}
//(new Rect (Screen.width * .5f, Screen.height * .55f, Screen.width * .5f, Screen.height * .1f)
					if (GUI.Button (new Rect (Screen.width * .575f, Screen.height * .65f, Screen.width * .2f, Screen.height * .1f), "End Game", oppaFontStyle)) {

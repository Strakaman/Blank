...
public class PlayerControllerScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        m_BoxCollider2D = GetComponent<BoxCollider2D>(); //get attached collider, store size and center
        m_SpriteRenderer = GetComponent<SpriteRenderer>();
        m_Rigidbody2D = GetComponent<Rigidbody2D>();
        s = m_BoxCollider2D.size;
        c = m_BoxCollider2D.offset;
    }
...
*/
    ...
    void damageProperties(GameObject collInfo, int damage, int knockback, float hitdelay)
    {
        ...
        if ((hittime + hitdelay < Time.time)/*&& PlayerInfo.g*/)
        {
            ...
            m_Rigidbody2D.AddForce(new Vector2(-horizontalPush, -verticalPush) * knockback);
